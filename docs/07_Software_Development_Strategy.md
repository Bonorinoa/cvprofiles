...
| Gate | Entry criterion | Exit criterion |
|---|---|---|
| G0 Docs | — | This suite reviewed; open questions listed |
| G1 Schemas | G0 | Pydantic models + fixture validates; freeze hash stable |
| G2 SCORE | G1 | Normalization + manifest; bad input fails loud |
| G3 RESTRICT | G2 | network/beta parse; invalid \(R\) rejected |
| G4 IDENTIFY slacks | G3 | Slack matrix matches hand-computed fixture |
| G5 \(M^*\) + range | G4 | Oracle DGP: membership + \([L,U]\) vs truth labels |
| G6 Bootstrap + \(\theta\)-grid | G5 | Seed-stable; H1–H3 directional |
| G7 REPORT | G6 | HTML/JSON/LaTeX from same bundle; empty-\(M^*\) beautiful |
| G8 Synth harness | G7 | Four metrics logged; CI can run mini battery |
| G9 Real baseline | G8 + user network | Frozen scores + USER network only |

**No GitHub requirement until G0 checkpoint.** No coding-agent free-for-all until `AGENTS.md` exists (deferred).
