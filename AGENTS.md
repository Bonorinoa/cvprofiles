...
- Package version: `2.5.1` — **v2.5.1 PyPI release (2026-08-09)**; ships the CLI holdout exposure, docs sync to shipped state, and the version-consistency CI check. v2.5.0 was the Rev 3 P1–P5 infrastructure checkpoint tagged 2026-08-08 (`corr_zero`/`monotone_rank`, `diff_means`/`map_distance`, stage + units-split holdout, coverage uncertainty band). P6 ships as its own tag; `v3.0.0` remains Gate C.
